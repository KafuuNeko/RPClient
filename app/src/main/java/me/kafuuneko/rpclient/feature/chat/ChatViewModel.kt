...
class ChatViewModel : CoreViewModelWithEvent<ChatUiIntent, ChatUiState>(
    ...
    private suspend fun onInit(intent: ChatUiIntent.Init) {
        ...
        val loaded = withContext(Dispatchers.IO) { loadNormalState(sessionId, inputDraft = "") }
        if (loaded == null) {
    ...
    private suspend fun buildSummaryRequest(
        ...
        while (true) {
            ...
            require(mChatRepository.getSummaryGenerationContext(sessionId, allowRefreshLatest, windowSize) == context) { "摘要素材已修改，请重试" }
    ...
    private suspend fun List<ChatMessage>.toDisplayMessageItems(
        ...
        ).map { it.copy(imageUuids = imageMap[it.id].orEmpty()) }
    }
